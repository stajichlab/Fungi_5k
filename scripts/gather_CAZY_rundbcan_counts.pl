...
for my $rundir ( readdir($ind) ) {
    ...
    print "$filepath\n";
    open(my $in => "pigz -dc $filepath |" ) || die "cannot open $filepath: $!";
#    my $header = <$in>;
    while(<$in>) {		# parse domtbl file
	next if /^\#/ || /^Gene/;		# skip comment lines
	chomp;
	#Gene_ID	EC	cazyme_fam	sub_fam	diamond_fam	Substrate	#ofTools
	my ($gene_name, $EC, $family, $subfam, $diamond_name, $substrate, $toolcount) = split(/\t/,$_);
	...
	print($family,"\n");
		...
		push @{$table{$species}}, [$gene_name,$hstart,$hend,
		$domain, $diamond_name,$substrate];
		$table_genes{$species}->{$gene_name}++;
		$specieset{$species}++;
	    }
	    #}
    }
