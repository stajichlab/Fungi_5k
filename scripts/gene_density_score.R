...
intergendist_p <- ggplot(densitystat_res) + 
  geom_point(aes(x=mean_gene_length,y=mean_dist,color=SUBPHYLUM,fill=SUBPHYLUM),size=1.5,alpha=0.5) +
  scale_colour_manual(values = mycolors) +
  scale_fill_manual(values = mycolors) +
  ylab("Mean Intergenic Distance") +
  xlab("Gene length") + ggtitle("Mean Gene Length vs Mean Intergenic Distance") +
  theme_cowplot(12) + theme(legend.position="bottom") + scale_y_log10() 
