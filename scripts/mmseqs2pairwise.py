...
def main():
    ...
    parser.add_argument('--tmpdir','--tmp', help='Temporary Directory for local DB')
    ...
    # Create or open the LevelDB database
    db = None
    if args.tmpdir:
        db = plyvel.DB(args.tmpdir, create_if_missing=True)
    gene2species = {}
    species_gene_count = {}
            ...
            with open(os.path.join(args.seqs,file),'r') as f:
                for line in f:                    
                    if line.startswith('>'):
                        gene = line.split()[0][1:]
                        if db:
                            # Store the gene-species mapping in the database
                            db.put(gene.encode(), species.encode())
                        else:
...
#            if args.debug:
#                t4 = time.time()
#                print(f"Reading and storing data for {file} [{species_gene_count[species]} proteins] took {t4-t3} seconds",file=sys.stderr)
