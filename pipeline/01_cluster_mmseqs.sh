...
time mmseqs cluster $DB ${DB}_cluster $SCRATCH --compressed 1 --min-seq-id 0.30 -c 0.7
#--cluster-reassign 1 --min-seq-id 0.30 --seq-id-mode 1 -c 0.70
mmseqs createtsv $DB $DB ${DB}_cluster results/${PREFIX}_cluster.tsv
pigz results/${PREFIX}_cluster.tsv
