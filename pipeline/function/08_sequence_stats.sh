...
if [ ! -s bigquery/$EXT.gz ]; then
    ...
    ls -U $INDIR | grep -P "\.fa$" | parallel -J $CPU ./scripts/calculate_AA_freq.py $INDIR/{} -v -o $SCRATCH/{.}.$EXT
...
if [ ! -s bigquery/$EXT.gz ]; then
    INDIR=input_cds
    ls -U $INDIR | grep -P "\.fa$" | parallel -J $CPU ./scripts/calculate_codon_freq.py $INDIR/{} -v -o $SCRATCH/{.}.$EXT
    FIRST=$(ls -U $SCRATCH/*.$EXT | head -n 1)
    head -n 1 $FIRST > bigquery/$EXT
...
if [ ! -s bigquery/$EXT.gz ]; then
    ...
    time ls -U $INDIR | grep -P "\.gff3$" | parallel -j $CPU ./scripts/build_genestats_bigquery.py $INDIR/{} --outdir $SCRATCH/{.}.${EXT}    
